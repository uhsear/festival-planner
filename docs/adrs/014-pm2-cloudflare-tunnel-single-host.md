...
PM2 is the process supervisor, and the ecosystem config uses `exec_mode: 'fork'` with
`instances: 1`. This ADR originally attributed that to the tsx-as-interpreter constraint of
ADR-007. That attribution is wrong as of 2026-09-09: the interpreter is now `node` and the entry
is the JavaScript bundle `dist/server.js`
(see [ADR-016](./016-esbuild-bundle-production-runtime.md)), and fork mode with one instance
stayed. The binding constraint is per-process state — send-idempotency in `lib/email.ts` and the
two in-memory limiters in `routes/email-auth.ts` — recorded at the `instances` line of
...
builds the backend bundle, builds the web bundle, restarts PM2, and gates on `/api/ready` before
tagging the deploy. The backend bundle build (`deploy.py` step 4) is the step whose failure aborts
the deploy earliest, while production is still wholly on the previous release.
...
- Trade-off: `exec_mode: 'fork'` with one instance means no CPU core parallelism at the Node.js
  level. Compiling to JS was tried and did not unlock `exec_mode: 'cluster'`
  (see [ADR-016](./016-esbuild-bundle-production-runtime.md)). Any future attempt at intra-host
  horizontal scaling must first move the per-process state out of `lib/email.ts` and
  `routes/email-auth.ts` into Redis. Do not start from the interpreter.
