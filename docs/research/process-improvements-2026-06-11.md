...
**P12 — Commit the deploy script and strip the credentials.** *What:* move `festie-deploy.py`/`festie-verify.py` into `scripts/deploy/`, read creds from env / a gitignored `.env.deploy`, document the runbook. *Evidence:* both live in `%TEMP%`, uncommitted, with a plaintext SSH password and prod login hardcoded (redacted here). *Effort:* M. *Impact:* high. *First step:* `mkdir scripts/deploy`, move + parametrize with `os.environ[...]`, add `.env.deploy` to `.gitignore`, **rotate both leaked passwords immediately**.
