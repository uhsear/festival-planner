...
- **Woov** (https://woov.com/) — closest competitor; personal timetable + artist-start push; reminders auto-update on schedule change; graceful pre-schedule lineup in organiser order; 3D map; find friends on map; AI-moderated chats; marquee announcements. Key lesson: dies without service → validates Festie offline-first.
