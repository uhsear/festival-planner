...
import createFestivalsRoutes from '../routes/festivals.js';

// ── Shared mock helpers ────────────────────────────────────────────────

function makeMockDeps(overrides: any = {}) {
  const deps = {
    express,
    config: {
      NODE_ENV: 'test',
    ...
    _hashSessionToken: (t: any) => `hashed-${t}`,
    hashSessionToken: (t: any) => `hashed-${t}`,
    createAuditLog: mock.fn(async () => {}),
    getRequestIp: () => '127.0.0.1',
    consumeUserAuthRateLimit: mock.fn(() => true),

    // account.js deps
    handleAvatarUpload: (_req: any, _res: any, next: any) => next(),
...
function createApp(router: any, prefix = '') {
  const app = express();
...
describe('routes/auth.js — createAuthRoutes', () => {
  ...
  describe('POST /login', () => {
    ...
    test('reactivates soft-deleted account on successful login', async () => {
      ...
      assert.equal((updateFn.mock.calls[0]!.arguments as any)[1].deletedAt, null);
    });
...
describe('routes/account.js — createAccountRoutes', () => {
  ...
  describe('POST /avatar', () => {
    ...
    test('uploads avatar successfully', async () => {
      ...
      const deps = makeMockDeps({
        handleAvatarUpload: (req: any, _res: any, next: any) => {
