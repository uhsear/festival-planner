...
function postImport(server, token, festivalId, body) {
...
async function countSets(festivalId) {
  const pool = new Pool({ connectionString: TEST_DATABASE_URL });
  try {
...
describe('lineup-import route (admin)', { skip, concurrency: 1 }, () => {
  test('CSV with header row parses artists, stages, days, times', async () => {
    ...
    const artists = res.body.data.sets.map((s) => s.artist);
  ...
  test('time normalization: 12-hour am/pm and short hours are padded to HH:mm', async () => {
    ...
    const noon = res.body.data.sets.find((s) => s.artist === 'Noon DJ');
    const night = res.body.data.sets.find((s) => s.artist === 'Night DJ');
  ...
  test('unknown stage defaults to first stage with a warning', async () => {
    ...
    assert.ok(res.body.data.warnings.some((w) => /unknown stage/i.test(w)));
  });
