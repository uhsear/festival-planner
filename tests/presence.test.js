...
const { createPresenceManager } = require('../lib/presence');
...
function makeSocket(overrides = {}) {
  const socket = {
    ...
    _rooms: new Set(),
    join(room) { socket._rooms.add(room); },
    leave(room) { socket._rooms.delete(room); },
    emit: mock.fn(),
    disconnect: mock.fn(),
  };
  return socket;
...
function makeIo(sockets = []) {
  ...
  return {
    _emitted,
    to: mock.fn(() => ({
      emit: mock.fn((...args) => _emitted.push(args)),
    })),
    emit: mock.fn(),
...
function makeRedisPresence(overrides = {}) {
  return {
