...
import { sanitizeString } from '../lib/helpers/sanitize';
...
describe('Hardening Tests', { concurrency: 1 }, () => {
  ...
  describe('Session Management', { concurrency: 1 }, () => {
    ...
    test('DELETE /api/v1/auth/sessions/:id revokes session', async () => {
      ...
      const nonCurrentSession = listRes.body.data.find((s: any) => !s.current);
    ...
    test('DELETE /api/v1/auth/sessions/:id prevents current session revocation', async () => {
      ...
      const listRes = await request(app)
        .get('/api/v1/auth/sessions')
        .set('x-user-token', token);
      const currentSessionId = listRes.body.data.find((s: any) => s.current)?.id;
  ...
  describe('Socket.IO Events', { concurrency: 1 }, () => {
    test('join:crew event validates crewId', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        clientSocket.on('connect_error', (err: any) => { clearTimeout(timeout); reject(err); });
      });
      ...
      const ackResult = await new Promise((resolve) => {
        ...
        clientSocket.emit('join:crew', { crewId: '' }, (response: any) => {
      ...
      if ((ackResult as any).error && (ackResult as any).error !== 'timeout') {
        assert.ok(true, 'Got error response for invalid crewId: ' + (ackResult as any).error);
      }
    ...
    test('join:festival event requires authentication', async () => {
      ...
      const ackResult = await new Promise((resolve) => {
        ...
        clientSocket.emit('join:festival', { festivalId: 'test-fest' }, (response: any) => {
          clearTimeout(timeout);
          resolve(response);
        });
      });
    ...
    test('leave:crew event removes socket from crew room', async () => {
      ...
      const joinAck = await new Promise((resolve) => {
        const timeout = setTimeout(() => resolve({ status: 'no-ack' }), 3000);
        clientSocket.emit('join:crew', { crewId: 'test-crew-123' }, (response: any) => {
          clearTimeout(timeout);
          resolve(response);
        });
      });
      ...
      const leaveAck = await new Promise((resolve) => {
        const timeout = setTimeout(() => resolve({ status: 'no-ack' }), 3000);
        clientSocket.emit('leave:crew', { crewId: 'test-crew-123' }, (response: any) => {
          clearTimeout(timeout);
          resolve(response);
        });
      });
      ...
      const inCrewRoom = rooms.some((r: any) => r.includes('test-crew-123'));
