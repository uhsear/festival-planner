...
import createHealthCoreRoutes from '../routes/health-core.js';
...
function makeDeps(overrides: any = {}) {
  ...
  return {
    ...
    sendError: (res: any, status: number, msg: string, code: string) => res.status(status).json({ data: null, error: { message: msg, status, code: code || 'ERROR' } }),
...
async function request(app: any, method: string, path: string, body?: any): Promise<any> {
  return new Promise((resolve, reject) => {
    const server = app.listen(0, '127.0.0.1', () => {
      ...
      req.on('error', (err: any) => { server.close(); reject(err); });
