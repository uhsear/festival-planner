...
const noopMw = (_req: any, _res: any, next: any) => next();

function makeLog() {
  return { info: noop, warn: noop, error: noop, debug: noop };
}

function makeIo(overrides: any = {}) {
...
function baseDeps(overrides: any = {}) {
  return {
    ...
    sanitizeIdentifier: (s: any) => (s || '').trim(),
...
function mountApp(router: any, prefix = '') {
  const app = express();
...
describe('routes/email-auth', () => {
  ...
  function buildEmailAuthDeps(overrides: any = {}) {
...
describe('routes/socket', () => {
  function buildSocketDeps(overrides: any = {}) {
    return {
      config: {
        NODE_ENV: 'test',
        SOCKET_JOIN_RATE_LIMIT: 100,
  ...
  function makeSocket(dataOverrides = {}) {
    ...
    return {
      id: 'socket-1',
      data: { connectionId: null, ...dataOverrides } as any,
      handshake: { headers: {} },
      rooms,
      join: mock.fn((room: any) => rooms.add(room)),
      leave: mock.fn((room: any) => rooms.delete(room)),
      emit: mock.fn(),
      on: mock.fn(),
      authenticated: false,
    };
  ...
  async function setupAndGetHandlers(deps: any) {
    const { default: setupSocketHandlers } = await import('../routes/socket.js');
    const io = deps.io;
    const handlers: Record<string, any> = {};

    io.on = mock.fn((event: any, handler: any) => {
  ...
  test('registers connection handler on io', async () => {
  ...
  test('connection handler sets connectionId on socket.data', async () => {
    const deps = buildSocketDeps();
    const handlers = await setupAndGetHandlers(deps);
  ...
  test('leave:festival — leaves room and emits presence', async () => {
    const deps = buildSocketDeps();
  ...
  test('leave:festival — rate limit rejection', async () => {
    const deps = buildSocketDeps();
    deps.consumeSocketRateLimit = mock.fn(() => false);
    const handlers = await setupAndGetHandlers(deps);
    const socket = makeSocket();
  ...
  test('disconnect — cleans up presence and metrics', async () => {
    const deps = buildSocketDeps();
    const handlers = await setupAndGetHandlers(deps);
    const socket = makeSocket();
    const socketHandlers: Record<string, any> = {};
  ...
  test('disconnect — handles disconnect without festivalId', async () => {
    const deps = buildSocketDeps();
    const handlers = await setupAndGetHandlers(deps);
    const socket = makeSocket();
    const socketHandlers: Record<string, any> = {};
...
describe('routes/crew-features', () => {
  function buildCrewFeaturesDeps(overrides: any = {}) {
    return baseDeps({
      stores: {
        pool: makePool(),
        crews: {
  ...
  async function mountCrewFeatures(deps: any) {
    const { default: mountCrewFeaturesFactory } = await import('../routes/crew-features.js');
...
describe('routes/expenses', () => {
  function buildExpenseDeps(overrides: any = {}) {
    return baseDeps({
      stores: {
        pool: makePool(),
        crews: {
  ...
  async function mountExpenses(deps: any) {
    const { default: createExpenseRoutes } = await import('../routes/expenses.js');
...
describe('routes/ratings', () => {
  function buildRatingDeps(overrides: any = {}) {
    return baseDeps({
      stores: {
        pool: {
  ...
  async function mountRatings(deps: any) {
    const { createRatingsRoutes } = await import('../routes/ratings.js');
...
describe('routes/calendar-sync', () => {
  function buildCalDeps(overrides: any = {}) {
    return baseDeps({
      stores: {
        pool: makePool(),
        profiles: {
  ...
  test('POST /calendar-sync/:festivalId — generates sync URL', async () => {
    ...
    const { default: createCalendarSyncRoutes } = await import('../routes/calendar-sync.js');
  ...
  test('GET /cal/:token.ics — returns ICS feed', async () => {
    const deps = buildCalDeps();
    const { createCalendarFeedRoute } = await import('../routes/calendar-sync.js');
...
describe('routes/lineup-import', () => {
  function buildLineupDeps(overrides: any = {}) {
    const storePool = makePool();
    return baseDeps({
      adminAuth: noopMw,
      setNoStore: noop,
  ...
  async function mountLineup(deps: any) {
    const { default: createLineupImportRoute } = await import('../routes/lineup-import.js');
...
describe('routes/weather', () => {
  function buildWeatherDeps(overrides: any = {}) {
    return baseDeps({
      stores: {
        pool: {
          query: mock.fn(async () => ({
  ...
  async function mountWeather(deps: any) {
    const { createWeatherRoutes } = await import('../routes/weather.js');
  ...
  test('GET /:festivalId — 404 when festival not found', async () => {
    ...
    deps.stores.pool.query = mock.fn(async () => ({ rows: [] }));
    const app = await mountWeather(deps);
  ...
  test('GET /:festivalId — handles fetch failure gracefully', async () => {
    ...
    (globalThis as any).fetch = mock.fn(async () => ({ ok: false }));
...
describe('routes/deep-links', () => {
  async function mountDeepLinks(configOverrides = {}) {
    const deps = baseDeps({
      config: { ...baseDeps().config, ...configOverrides },
    });
    const { default: createDeepLinkRoutes } = await import('../routes/deep-links.js');
    const router = createDeepLinkRoutes(deps);
    return mountApp(router);
  }

  test('GET /apple-app-site-association — returns AASA when configured', async () => {
    const app = await mountDeepLinks({ APPLE_TEAM_ID: 'REALTEAMID' });
  ...
  test('GET /apple-app-site-association — 503 for placeholder TEAMID', async () => {
    const app = await mountDeepLinks({ APPLE_TEAM_ID: 'TEAMID' });

    await request(app).get('/apple-app-site-association').expect(503);
  });

  test('GET /assetlinks.json — returns asset links when configured', async () => {
    const app = await mountDeepLinks({ ANDROID_CERT_FINGERPRINTS: 'AA:BB,CC:DD' });
...
describe('routes/analytics-install', () => {
  async function mountAnalytics(overrides: any = {}) {
    const deps = baseDeps(overrides);
    const { default: createAnalyticsInstallRoutes } = await import('../routes/analytics-install.js');
...
describe('routes/client-metrics', () => {
  async function mountMetrics(overrides: any = {}) {
    const deps = baseDeps({
      promMetrics: null, // No prom-client in test
      ...overrides,
    });
    const { default: createClientMetricsRoutes } = await import('../routes/client-metrics.js');
...
describe('routes/activity', () => {
  async function mountActivity(overrides: any = {}) {
    const deps = baseDeps({
      stores: {
        pool: makePool(),
        crews: {
    ...
    const { default: createActivityRoutes } = await import('../routes/activity.js');
