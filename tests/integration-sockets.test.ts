...
describe('Integration — Sockets', { concurrency: 1 }, () => {
  ...
  test('rate limits websocket connection attempts per IP', async () => {
    ...
    const connectError: any = await waitForEvent(third, 'connect_error', (error: any) => String(error?.message || error) === 'websocket error');
  ...
  test('covers presence and realtime profile events', async () => {
    ...
    const alicePresence: any = await presenceForAlice;
    ...
    await server.request
      ...
      .expect(200);
    const updatedPayload: any = await updatedProfile;
  ...
  test('join:crew — user joins a crew room and receives ack', async () => {
    ...
    const ack: any = await new Promise((resolve, reject) => {
      ...
      socket.emit('join:crew', { crewId }, (response: any) => {
  ...
  test('leave:crew — user leaves a crew room', async () => {
    ...
    const joinAck: any = await new Promise((resolve, reject) => {
      const timeout = setTimeout(() => reject(new Error('join:crew ack timeout')), 4000);
      socket.emit('join:crew', { crewId }, (response: any) => {
        clearTimeout(timeout);
        resolve(response);
    ...
    const rejoinAck: any = await new Promise((resolve, reject) => {
  ...
  test('reconnect:restore — restores festival room subscription on reconnect', async () => {
    ...
    const restoreAck: any = await new Promise((resolve, reject) => {
      const timeout = setTimeout(() => reject(new Error('reconnect:restore ack timeout')), 4000);
      socket2.emit('reconnect:restore', { festivalId: 'fest-1', userToken: alice.token }, (response: any) => {
        clearTimeout(timeout);
        resolve(response);
      });
    });
