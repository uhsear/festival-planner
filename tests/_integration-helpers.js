...
/**
 * Shared setup for integration-*.test.js files (split from tests/integration.test.js).
...
async function seedTestData(databaseUrl, festivals = createFestivalFixture()) {
  const pool = new Pool({ connectionString: databaseUrl });
  try {
...
async function startServer(overrides = {}) {
  ...
  const address = planner.server.address();
...
async function loginUser(server, username, password = DEFAULT_PASSWORD) {
  const response = await server.request
...
async function joinFestivalProfile(server, userToken, festivalId = 'fest-1') {
  const response = await server.request
...
function waitForEvent(emitter, event, predicate = () => true, timeoutMs = 4_000) {
...
function markTrustedMutation(requestBuilder) {
  return requestBuilder.set(TRUSTED_MUTATION_HEADER, '1');
}

async function uploadAvatar(server, token) {
