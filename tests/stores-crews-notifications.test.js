...
const createCrewsStore = require('../lib/db/stores/crews');
const createNotificationsStore = require('../lib/db/stores/notifications');
const { createRatingsStore } = require('../lib/db/stores/ratings');
const createRolesStore = require('../lib/db/stores/roles');
...
function mockPoolThatThrows(error) {
  return {
    queries: [],
...
describe('createCrewsStore — topicSubscriptions', () => {
  it('getForUser returns empty object when no rows', async () => {
    ...
    const { topicSubscriptions } = createCrewsStore(pool);
...
describe('createCrewsStore — meetingPoints', () => {
  it('create inserts and returns the meeting point', async () => {
    ...
    const { meetingPoints } = createCrewsStore(pool);
...
describe('createNotificationsStore — deviceTokens', () => {
  it('register inserts token with correct params', async () => {
    const pool = mockPool([]);
    const { deviceTokens } = createNotificationsStore(pool);
  ...
  it('listByUsers returns empty array for null/undefined', async () => {
    ...
    assert.deepStrictEqual(await deviceTokens.listByUsers(null), []);
    assert.deepStrictEqual(await deviceTokens.listByUsers(undefined), []);
  });
...
describe('createNotificationsStore — notificationPrefs', () => {
  it('get returns prefs when found', async () => {
    ...
    const { notificationPrefs } = createNotificationsStore(pool);
...
describe('createNotificationsStore — notificationLog', () => {
  it('insert stores all entry fields', async () => {
    const pool = mockPool([]);
    const { notificationLog } = createNotificationsStore(pool);
...
describe('createNotificationsStore — notificationCounts', () => {
  it('getByUser returns counts for user', async () => {
    ...
    const { notificationCounts } = createNotificationsStore(pool);
