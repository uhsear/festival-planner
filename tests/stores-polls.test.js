...
describe('lib/db/stores/polls.js', () => {
  let createPollsStore;

  beforeEach(() => {
    const storePath = require.resolve('../lib/db/stores/polls');
    delete require.cache[storePath];
    createPollsStore = require('../lib/db/stores/polls');
  });
  ...
  describe('create', () => {
    it('inserts a poll and returns the created row', async () => {
      ...
      assert.ok(call.arguments[0].includes('INSERT INTO crew_polls'));
      assert.ok(call.arguments[0].includes('RETURNING'));
  ...
  describe('listByCrew', () => {
    it('returns polls for a crew with parsed options and votes', async () => {
      ...
      assert.deepStrictEqual(pool.query.mock.calls[0].arguments[1], ['crew-1']);
    });
    ...
    it('returns empty array when crew has no active polls', async () => {
      ...
      assert.deepStrictEqual(pool.query.mock.calls[0].arguments[1], ['crew-empty']);
    });
  ...
  describe('vote', () => {
    it('inserts a vote and returns the created row', async () => {
      ...
      assert.strictEqual(pool.query.mock.calls.length, 1);
      const call = pool.query.mock.calls[0];
      assert.ok(call.arguments[0].includes('INSERT INTO crew_poll_votes'));
      assert.deepStrictEqual(call.arguments[1], ['poll-1', 'user-1', 0]);
    });
  ...
  describe('getResults', () => {
    it('returns poll with aggregated votes', async () => {
      ...
      assert.deepStrictEqual(pool.query.mock.calls[0].arguments[1], ['poll-1']);
    });
  ...
  describe('close', () => {
    it('marks a poll as closed and returns the updated row', async () => {
      ...
      assert.strictEqual(pool.query.mock.calls.length, 1);
      const call = pool.query.mock.calls[0];
      assert.ok(call.arguments[0].includes('UPDATE crew_polls'));
      assert.ok(call.arguments[0].includes('closed = TRUE'));
      assert.ok(call.arguments[0].includes('RETURNING'));
      assert.deepStrictEqual(call.arguments[1], ['poll-1']);
    });
  ...
  describe('countActiveByCrew', () => {
    it('returns the count of active polls for a crew', async () => {
      ...
      const call = pool.query.mock.calls[0];
      assert.ok(call.arguments[0].includes('SELECT COUNT(*)'));
      assert.ok(call.arguments[0].includes('closed = FALSE'));
      assert.deepStrictEqual(call.arguments[1], ['crew-1']);
    });
