...
function fakeRedis(overrides = {}) {
  const store = new Map();
  return {
    pipeline(results) { return fakePipeline(overrides.pipelineResults || [[null, 1], [null, -1]]); },
    async incr(key) { const v = (store.get(key) || 0) + 1; store.set(key, v); return v; },
    async pexpire() { return 1; },
    async hset(key, field, val) { if (!store.has(key)) store.set(key, {}); store.get(key)[field] = val; },
    async hdel(key, field) { if (store.has(key)) delete store.get(key)[field]; },
    async hgetall(key) { return store.get(key) || {}; },
    async expire() { return 1; },
    async get(key) { return store.get(key) || null; },
    async setex(key, ttl, val) { store.set(key, val); },
    ...
    duplicate(opts) {
      return {
        on(evt, cb) { if (evt === 'message') this._msgCb = cb; },
        subscribe(ch1, ch2, cb) { if (cb) cb(null); },
        async unsubscribe() {},
        disconnect() {},
        _msgCb: null,
      };
...
describe('redis: createRedisPresenceStore', () => {
  ...
  it('removeBySocketId clears from multiple festivals', async () => {
    ...
    const redis = fakeRedis({
      pipeline() {
        return {
          hdel(key, field) { calls.push({ key, field }); return this; },
          async exec() { return []; },
        };
      },
    });
...
describe('redis: createCacheInvalidationBus', () => {
  ...
  it('publishUserInvalidation calls redis.publish', async () => {
    ...
    const redis = fakeRedis({
      async publish(ch) { publishedChannel = ch; return 1; },
      duplicate() {
        return {
          on() {},
          subscribe(ch1, ch2, cb) { if (cb) cb(null); },
