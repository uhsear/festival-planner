...
async function seedFestival(festival: any) {
  const pool = new Pool({ connectionString: TEST_DATABASE_URL });
  try {
    await pool.query(
...
const createdFestivalIds = new Set<string>();
...
async function startServer(overrides: any = {}) {
  ...
  return {
    ...
    async close() {
      ...
      else if (planner.server) await new Promise<void>((r: any) => planner.server.close(r));
    },
  };
}
...
async function joinFestival(server: any, token: any, festivalId: any) {
  const res = await server.request
...
async function grantAdminTo(username: any) {
  const pool = new Pool({ connectionString: TEST_DATABASE_URL });
  try {
    await pool.query(
