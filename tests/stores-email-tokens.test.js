...
describe('lib/db/stores/email-tokens.js', () => {
  let createEmailTokensStore;

  beforeEach(() => {
    const storePath = require.resolve('../lib/db/stores/email-tokens');
    delete require.cache[storePath];
    createEmailTokensStore = require('../lib/db/stores/email-tokens').createEmailTokensStore;
  });
  ...
  describe('findUserByEmail', () => {
    ...
    it('returns null when no user matches the email', async () => {
      ...
      assert.deepStrictEqual(pool.query.mock.calls[0].arguments[1], ['nobody@example.com']);
    });
  });
