...
describe('lib/db/stores/festivals.js', () => {
  ...
  let createFestivalsStore: any;

  beforeEach(async () => {
    const mod = await import('../lib/db/stores/festivals.js');
    createFestivalsStore = mod.default ?? mod;
  });

  function makeFestivalData(overrides: any = {}) {
    return {
      id: 'fest-1',
      name: 'Test Festival',
      location: 'Miami',
  ...
  function makeTransactionPool(clientQueryResults: any[] = []) {
  ...
  describe('getById', () => {
    it('returns matching festival via direct query', async () => {
      ...
      assert.equal(pool.query.mock.calls.length, 1);
      assert.ok((pool.query.mock.calls[0]! as any).arguments[0].includes('festivals'));
    });
  ...
  describe('create', () => {
    ...
    it('uses default createdAt and b2bSeparator when not provided', async () => {
      ...
      const insertCall = client.query.mock.calls[1]! as any;
  ...
  describe('update', () => {
    ...
    it('updates b2bSeparator, latitude, longitude', async () => {
      ...
      const updateCall = client.query.mock.calls[1]! as any; // after BEGIN
    ...
    it('updates stages when provided', async () => {
      ...
      const deleteCall = client.query.mock.calls.find(
        (c: any) => typeof c.arguments[0] === 'string' && c.arguments[0].includes('DELETE FROM festival_stages'),
      );
  ...
  describe('replaceAll', () => {
    it('deletes all festivals when passed empty array', async () => {
      ...
      const deleteCall = client.query.mock.calls.find(
        (c: any) => typeof c.arguments[0] === 'string' && c.arguments[0].includes('DELETE FROM festivals'),
      );
...
// ---------------------------------------------------------------------------
describe('lib/shutdown.js — additional coverage', async () => {
  const { createBackgroundTasks, createCloseHandler } = await import('../lib/shutdown.js');

  describe('memory pressure monitoring (lines 107-119)', () => {
    let ctx: any, ioStub: any;
  ...
  describe('createCloseHandler — in-flight request wait (lines 204-205)', () => {
    it('waits for in-flight requests to drain within timeout', async () => {
      ...
      const deps = {
        ...
        io: {
          engine: { clientsCount: 0, close: mock.fn((cb: any) => cb()) },
          emit: mock.fn(),
          of: () => ({ sockets: new Map() }),
        },
        config: {
...
// ---------------------------------------------------------------------------
describe('lib/socket-setup.js — allowRequest coverage', async () => {
  ...
  it('allowRequest rejects when rate limit exceeded', (_, done) => {
    ...
    const { io } = configureSocketIO({} as any, ctx);
    toClose.push({ io, server: io.httpServer });

    // Access the allowRequest option
    const opts = (io as any)._opts || (io as any).opts;
    if (opts && opts.allowRequest) {
      const fakeReq = {
        url: '/',
        headers: { host: 'localhost' },
      };
      opts.allowRequest(fakeReq, (errMsg: any, allowed: any) => {
  ...
  it('allowRequest accepts valid query token auth', (_, done) => {
    ...
    if (opts && opts.allowRequest) {
      ...
      opts.allowRequest(fakeReq, (err: any, allowed: any) => {
  ...
  it('auth timeout disconnects unauthenticated socket', async () => {
    ...
    assert.equal((testSocket.once.mock.calls[0]! as any).arguments[0], 'disconnect');
  });
...
describe('lib/reset-pages.js', async () => {
  ...
  describe('renderResetFormPage', () => {
    it('returns HTML string with DOCTYPE', () => {
      const html = renderResetFormPage('tok123', 'https://festie.us', 'nonce');
      assert.ok(html.startsWith('<!DOCTYPE html>'));
    });

    it('includes the token in a script tag', () => {
      const html = renderResetFormPage('my-token', 'https://festie.us', 'nonce');
      assert.ok(html.includes('"my-token"'));
    });

    it('includes the origin in a script tag', () => {
      const html = renderResetFormPage('tok', 'https://festie.us', 'nonce');
      assert.ok(html.includes('"https://festie.us"'));
    });

    it('escapes origin in the HTML link', () => {
      const html = renderResetFormPage('tok', 'https://festie.us?a=1&b=2', 'nonce');
  ...
  describe('renderResetErrorPage', () => {
    it('returns HTML string with DOCTYPE', () => {
      const html = renderResetErrorPage('Link expired', 'nonce');
      assert.ok(html.startsWith('<!DOCTYPE html>'));
    });

    it('includes the error message (escaped)', () => {
      const html = renderResetErrorPage('Token <expired> & invalid', 'nonce');
...
describe('lib/swagger-ui-setup.js', () => {
  let mountSwaggerUI: any;

  beforeEach(async () => {
    const mod = await import('../lib/swagger-ui-setup.js');
    mountSwaggerUI = mod.mountSwaggerUI;
  });
  ...
  it('returns early (no-op) when swagger-ui-dist is not installed', () => {
    ...
    const app = {
      ...
      get: (...args: any[]) => getCalls.push(args),
    };
  ...
  it('GET /api/docs handler sends HTML with swagger UI', () => {
    ...
    assert.equal((res.type.mock.calls[0]! as any).arguments[0], 'html');
...
describe('routes/spotify.js', () => {
  let createSpotifyRoutes: any;

  beforeEach(async () => {
    const mod = await import('../routes/spotify.js');
    createSpotifyRoutes = mod.default ?? mod;
  });
  ...
  it('returns 404 when set not found', async () => {
    ...
    assert.equal((deps.sendError.mock.calls[0]! as any).arguments[1], 404);
  });
  ...
  it('falls back to artist embed when search response is not ok', async () => {
    ...
    global.fetch = mock.fn(async () => {
      fetchCallCount++;
      if (fetchCallCount === 1) {
        return { ok: true, status: 200, json: async () => ({ access_token: 'tok-fallback', expires_in: 3600 }) };
      }
  ...
  it('handles error in route handler gracefully', async () => {
    ...
    await handler(req, res);
    assert.equal(deps.sendError.mock.calls.length, 1);
    assert.equal((deps.sendError.mock.calls[0]! as any).arguments[1], 500);
  });
