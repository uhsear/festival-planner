...
function makeDeps(overrides: any = {}) {
  const deps: any = {
    ...
    getUserById: mock.fn(async (id: any) => {
    ...
    serializeExportCrewProfile: (p: any) => ({ id: p.id, name: p.name || 'anon' }),
    ...
    stores: {
      ...
      users: {
        ...
        hardDelete: mock.fn(async (id: any) => ({ id, username: 'deleted-user', avatarKey: null })),
        delete: mock.fn(async (id: any) => ({ id, username: 'deleted-user', avatarKey: null })),
        countActive: mock.fn(async () => 2),
      },
      roles: {
        getUserRoles: mock.fn(async () => ['admin']),
        getUserRolesBatch: mock.fn(async (ids: any) => new Map(ids.map((id: any) => [id, ['admin']]))),
      ...
      profiles: {
        ...
        readByUserAndFestival: mock.fn(async (userId: any, festivalId: any) => {
      ...
      crews: {
        ...
        getMember: mock.fn(async (crewId: any, userId: any) => {
...
function buildApp(router: any, prefix = '') {
  const app = express();
...
describe('routes/admin-status.js', () => {
  test('factory returns an object with a router', async () => {
  ...
  test('GET /admin/status includes CSP header with nonce', async () => {
    ...
    deps.adminAuth = (req: any, res: any, next: any) => {
...
describe('routes/admin-metrics.js', () => {
  ...
  test('GET /internal/metrics-json returns 403 for non-localhost', async () => {
    ...
    appWithIp.use((req: any, _res: any, next: any) => {
...
describe('routes/admin-users.js', () => {
  async function buildUserRouter(overrides: any = {}) {
    const { default: mountAdminUserRoutes } = await import('../routes/admin-users.js');
    const deps = makeDeps(overrides);
    const router = express.Router();
    const crypto = await import('crypto');
    const { parsePageParams, paginateArray } = await import('../lib/pagination.js');
    const passwordResetRateLimit = (req: any, res: any, next: any) => next();
    const adminWriteLimit = (req: any, res: any, next: any) => next();
...
describe('routes/admin-bulk.js', () => {
  async function buildBulkRouter(overrides: any = {}) {
  ...
  test('POST /bulk/deactivate handles partial failures', async () => {
    ...
    const { default: mountAdminBulkRoutes } = await import('../routes/admin-bulk.js');
    const router = express.Router();
    mountAdminBulkRoutes({ router, deps: deps2, ctx: { adminWriteLimit: (req: any, res: any, next: any) => next() } });
...
describe('routes/admin.js', () => {
  ...
  async function buildAdminApp(overrides: any = {}) {
    const { default: createAdminRoutes } = await import('../routes/admin.js');
    const deps = makeDeps(overrides);
    const router = createAdminRoutes(deps);
    return { app: buildApp(router), deps };
  }

  test('factory returns a router', async () => {
    const { default: createAdminRoutes } = await import('../routes/admin.js');
    const deps = makeDeps();
    const router = createAdminRoutes(deps);
    assert.ok(router);
...
describe('routes/export.js', () => {
  ...
  async function buildExportApp(overrides: any = {}) {
...
describe('routes/notifications.js', () => {
  async function buildNotifApp(overrides: any = {}) {
