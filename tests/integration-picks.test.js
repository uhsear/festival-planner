...
describe('Integration — Picks', { concurrency: 1 }, () => {
  test('hides other users notes and restricts export ownership', async () => {
    ...
    const visibleBob = profileList.body.data.find((profile) => profile.id === bobProfile.id);
    const visibleAlice = profileList.body.data.find((profile) => profile.id === aliceProfile.id);
    ...
    assert.match(exportResponse.headers['content-disposition'], /schedule\.html/);
    assert.match(exportResponse.headers['content-disposition'], /filename\*=UTF-8''/);
    assert.match(exportResponse.headers['content-security-policy'], /style-src-attr 'unsafe-inline'/);
  ...
  test('persists sessions and profile data across server restart', async () => {
    ...
    const first = createFestivalPlanner({
    ...
    const second = createFestivalPlanner({
      DATABASE_URL: TEST_DATABASE_URL,
      PUBLIC_DIR,
      NODE_ENV: 'test',
      REDIS_ENABLED: 'false',
    ...
    try {
      ...
      const profiles = await secondRequest
        ...
        .set('x-user-token', token)
        .expect(200);
      const ownProfile = profiles.body.data.find((entry) => entry.id === profile.body.data.id);
  ...
  test('concurrent profile updates dont corrupt data', async () => {
    ...
    const profiles = await server.request
      ...
      .set('x-user-token', alice.token)
      .expect(200);
    const ownProfile = profiles.body.data.find((p) => p.id === profile.id);
...
describe('profile soft-delete filtering', { concurrency: 1 }, () => {
    ...
    try {
      ...
      if (res.status === 200 && res.body.data) {
        ...
        const found = Array.isArray(res.body.data)
          ? res.body.data.find((p) => p.id === profile.id)
  ...
  test('soft-deleted profiles do not appear in pick overlap queries', async () => {
    ...
    try {
      ...
      const bobInResults = profiles.find((p) => p.id === bobProfile.id);
  ...
  test('soft-deleted profiles do not appear in profiles list endpoint', async () => {
    ...
    try {
      ...
      const deletedInResults = profilesAfter.find((p) => p.id === p2.id);
