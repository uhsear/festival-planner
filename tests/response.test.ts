...
import { ErrorCodes, sendSuccess, sendError, RETRYABLE_CODES } from '../lib/response.js';
...
describe('response: ErrorCodes', () => {
  it('contains all expected error codes', () => {
    ...
    for (const code of expected) {
      ...
      assert.equal((ErrorCodes as any)[code], code);
    }
  });
...
describe('response: sendSuccess', () => {
  ...
  it('trims meta to 10 keys when it has more', () => {
    const res = createMockRes();
    const bigMeta: Record<string, number> = {};
  ...
  it('sets X-API-Version header when config has API_VERSION', () => {
    const res = createMockRes();
    sendSuccess(res, {}, null, { API_VERSION: '2' } as any);
