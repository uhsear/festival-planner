...
const { createAvatarHelpers } = require('../lib/app-context/avatar');
...
function makeSendError() {
  const calls = [];
  const fn = (res, status, msg, code) => { calls.push({ status, msg, code }); };
  fn.calls = calls;
  return fn;
}
...
describe('avatar: ensureAvatarDir', () => {
  ...
  it('cleans up .tmp files on ensureAvatarDir call', () => {
    ...
    const remaining = fs.readdirSync(dirPath).filter((f) => f.endsWith('.tmp'));
