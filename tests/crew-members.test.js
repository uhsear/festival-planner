...
function makeDeps(overrides = {}) {
  ...
  const deps = {
    ...
    validateQuery: overrides.validateQuery || (() => (req, _res, next) => { req.validatedQuery = req.query; next(); }),
    schemas: {
      crewUserSearchQuery: {},
      crewTransfer: {},
      crewAddMember: {},
...
function buildApp(overrides = {}) {
  ...
  const createCrewMemberRoutes = require('../routes/crew-members');
...
describe('routes/crew-members.js — GET /search-users', () => {
  ...
  test('escapes LIKE metacharacters in search query', async () => {
    ...
    const param = poolQuery.mock.calls[0].arguments[1][0];
...
describe('routes/crew-members.js — DELETE /:crewId/leave', () => {
  ...
  test('emits crew:member-left via Socket.IO on leave', async () => {
    ...
    assert.equal(ioObj._emit.mock.calls[0].arguments[0], 'crew:member-left');
...
describe('routes/crew-members.js — DELETE /:crewId/members/:userId', () => {
  ...
  test('emits crew:member-kicked via Socket.IO', async () => {
    ...
    assert.equal(ioObj.to.mock.calls.length, 1);
    assert.equal(ioObj.to.mock.calls[0].arguments[0], 'crew:crew-1');
    assert.equal(ioObj._emit.mock.calls[0].arguments[0], 'crew:member-kicked');
    assert.deepEqual(ioObj._emit.mock.calls[0].arguments[1], {
      crewId: 'crew-1',
      userId: 'user-2',
    });
...
describe('routes/crew-members.js — PUT /:crewId/transfer', () => {
  ...
  test('transfers ownership successfully', async () => {
    ...
    assert.equal(updateMemberRole.mock.calls[0].arguments[2], 'owner');
    assert.equal(updateMemberRole.mock.calls[1].arguments[1], 'user-1');
    assert.equal(updateMemberRole.mock.calls[1].arguments[2], 'member');
  });
  ...
  test('emits crew:updated via Socket.IO without inviteCode', async () => {
    ...
    assert.equal(ioObj.to.mock.calls.length, 1);
    assert.equal(ioObj._emit.mock.calls[0].arguments[0], 'crew:updated');
    const broadcastData = ioObj._emit.mock.calls[0].arguments[1];
  ...
  test('returns 400 when target userId is missing', async () => {
    const { app } = buildApp({
      sanitizeIdentifier: (s) => (typeof s === 'string' && s.trim() ? s.trim() : ''),
      resolveCrewOwnership: mock.fn(async () => ({
        crew: { ...DEFAULT_CREW },
        membership: { role: 'owner' },
      })),
