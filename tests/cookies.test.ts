...
import { createCookieHelpers } from '../lib/app-context/cookies.js';
...
function makeMockRes() {
  const state: any = { headers: {}, cookies: {}, cleared: [] };
  return {
    setHeader(name: string, val: any) { state.headers[name] = val; },
    cookie(name: string, val: string, opts: any) { state.cookies[name] = { val, opts }; },
    clearCookie(name: string, opts: any) { state.cleared.push({ name, opts }); },
    _state: state,
  };
}

function makeMockReq(overrides: any = {}) {
  return {
