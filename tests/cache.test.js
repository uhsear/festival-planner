...
const { createCacheHelpers } = require('../lib/app-context/cache');

// ─── Mock Stores ─────────────────────────────────────────────────────────────

function makeStores(data = {}) {
  ...
  return {
    ...
    profiles: {
      ...
      getById: async (id) => profiles.find((p) => p.id === id) || null,
      readByUserAndFestival: async (userId, festivalId) =>
        profiles.find((p) => p.userId === userId && p.festivalId === festivalId) || null,
    },
  };
}
...
describe('cache: getUserFestivalProfile fallback', () => {
  it('falls back to scanning all profiles when store lacks readByUserAndFestival', async () => {
    const stores = makeStores();
    delete stores.profiles.readByUserAndFestival;
    delete stores.profiles.getById;
    const cache = createCacheHelpers({ stores, redis: null, log: noopLog });
    const profile = await cache.getUserFestivalProfile('u1', 'f1');
    assert.equal(profile.id, 'p1');
  });
