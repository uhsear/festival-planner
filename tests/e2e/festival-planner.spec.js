...
async function openApp(page, app, suffix = '') {
  await page.goto(`${app.baseUrl}${suffix}`);
  await expect(page.locator('#app')).toBeVisible();
}

async function registerUser(page, app, username, password = DEFAULT_PASSWORD) {
...
async function joinFestival(page) {
...
async function loginUser(page, app, username, password = DEFAULT_PASSWORD) {
  await openApp(page, app);
  await page.locator('#authUsername').fill(username);
  await page.locator('#authPassword').fill(password);
  await page.locator('#authBtn').click();
...
async function openSet(page, artist) {
...
async function uploadAvatarInMenu(page) {
...
test.describe('festival planner browser regression', () => {
  test('keeps planner features working while sessions stay out of localStorage', async ({ app, page }) => {
  ...
  test('updates group and presence across browsers', async ({ app, browser }) => {
  ...
  test('keeps planning and live controls usable on mobile', async ({ app, browser }) => {
  ...
  test('lets admins create, edit, and delete festivals through the UI', async ({ app, page }) => {
  ...
  test('recovers cleanly when the current festival is deleted', async ({ app, page }) => {
    await registerUser(page, app, 'owner');
    await joinFestival(page);
    await loginAdmin(page);
    await openAdminPanel(page);
  ...
  test('allows guest browsing without authentication', async ({ app, page }) => {
  ...
  test('share link pages render without authentication', async ({ app, page }) => {
  ...
  test('forgot password flow shows correct UI states', async ({ app, page }) => {
  ...
  test('WCAG: all interactive elements meet 44px touch target minimum', async ({ app, page }) => {
  ...
  test('WCAG: tab elements have aria-controls', async ({ app, page }) => {
  ...
  test('WCAG: auth errors are announced via role=alert', async ({ app, page }) => {
    await openApp(page, app);
  ...
  test('account menu shows email and supports email change flow', async ({ app, page }) => {
  ...
  test('lets admins reset and delete users without exposing passwords', async ({ app, browser, page }) => {
    const bobContext = await browser.newContext();
    const bobPage = await bobContext.newPage();
