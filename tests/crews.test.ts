...
async function joinFestival(server: any, userToken: any, festivalId = server.festivalId) {
  const res = await server.request
...
async function createCrew(server: any, token: any, name = `Crew ${Date.now()}-${Math.random().toString(36).slice(2, 6)}`) {
  const res = await server.request
    .post('/api/v1/crews')
...
describe('crews routes', { concurrency: 1, skip }, () => {
  ...
  test('GET /api/v1/crews/:crewId exposes members with userId/username/role/joinedAt', async () => {
    ...
    const mRow = res.body.data.members.find((m: any) => m.userId === member.id);
  ...
  test('DELETE /api/v1/crews/:crewId as owner removes crew', async () => {
    ...
    const listRes = await server.request
      ...
      .set('x-user-token', owner.token)
      .expect(200);
    assert.ok(!listRes.body.data.some((c: any) => c.id === crew.id),
