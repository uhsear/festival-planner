...
import { withTransaction, parseJsonObject, serializeJson, openPlannerDatabase } from '../lib/db/connection.js';
import createUtils from '../lib/db/utils.js';
import { createStores, createDbLatencyTracker } from '../lib/db/index.js';
...
function mockPoolThatFailsOnQuery(failOn: any): any {
  const queries: any[] = [];
  const released: any[] = [];
  return {
    queries,
...
describe('createStores', () => {
  ...
  it('stores have object-typed values for all data stores', () => {
    ...
    for (const key of objectStores) {
      assert.equal(typeof (stores as any)[key], 'object', `${key} should be an object`);
      assert.notEqual((stores as any)[key], null, `${key} should not be null`);
    }
  });
});
...
describe('createDbLatencyTracker', () => {
  ...
  it('returns stats object and wrapStore function', () => {
    const tracker = createDbLatencyTracker(mockLog);
  ...
  it('wraps store functions to track latency', async () => {
    const tracker = createDbLatencyTracker(mockLog);
    const store = {
      findById: async (id: any) => ({ id, name: 'test' }),
      name: 'users',
    };
