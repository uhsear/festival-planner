...
describe('Crew System', { concurrency: 1 }, () => {
  test('crew lifecycle: create, invite, join, overlap, transfer, leave, kick, delete', async () => {
    ...
    const listedCrew = listRes.body.data.find((c: any) => c.id === crew.id);
    ...
    const bobRole = transferred.members.find((m: any) => m.userId === bob.user.id);
    const aliceRole = transferred.members.find((m: any) => m.userId === alice.user.id);
