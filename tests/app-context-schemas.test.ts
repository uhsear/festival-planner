...
describe('schemas: validateParams middleware', () => {
  it('sets req.validatedParams on success and calls next', () => {
    ...
    const req: any = { params: { crewId: 'crew-abc123' } };
  ...
  it('calls sendError on invalid params', () => {
    ...
    const res: any = {
      setHeader() {},
      status(s: any) { statusSet = s; return res; },
  ...
  it('validates compound params (crewId + expenseId)', () => {
    ...
    const req: any = { params: { crewId: 'c-1', expenseId: 'e-1' } };
    let nextCalled = false;
...
describe('schemas: sanitizeFestivalPayload', () => {
  ...
  const mockCreateOpaqueId = (prefix: any) => `${prefix}-mock-${idCounter++}`;
...
describe('app-context: runUserTask pattern', () => {
  let userTaskQueues: Map<string, Promise<any>>;
...
describe('app-context: payload normalizer wrappers', () => {
  it('normalizePickPayload wrapper curries config', () => {
    ...
    const wrapper = (input: any) => normalizePickPayload(input, config);
  ...
  it('normalizeNotePayload wrapper curries config', () => {
    ...
    const wrapper = (input: any) => normalizeNotePayload(input, config);
  ...
  it('normalizeReminderPayload wrapper curries config', () => {
    ...
    const wrapper = (input: any) => normalizeReminderPayload(input, config);
  ...
  it('sanitizeFestivalPayload wrapper curries config and createOpaqueId', () => {
    ...
    let counter = 0;
    const createOpaqueId = (prefix: any) => `${prefix}-${counter++}`;
    const wrapper = (input: any, existing: any) => sanitizeFestivalPayload(input, existing, config, createOpaqueId);
