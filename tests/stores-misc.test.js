...
// Helpers
// ---------------------------------------------------------------------------
const { parseJsonObject } = require('../lib/db/connection');
...
// 2. Audit Store
// ===========================================================================
const createAuditStore = require('../lib/db/stores/audit');
...
describe('ExpensesStore', () => {
  ...
  describe('getBalances()', () => {
    ...
    it('calculates balances with specific splitWith', async () => {
      ...
      assert.equal(result.find(r => r.userId === 'u1').balance, 50);
      assert.equal(result.find(r => r.userId === 'u2').balance, -50);
      assert.equal(result.find(r => r.userId === 'u3').balance, 0);
    });
    ...
    it('handles invalid JSON split_with string gracefully', async () => {
      ...
      assert.equal(result.find(r => r.userId === 'u1').balance, 30);
      assert.equal(result.find(r => r.userId === 'u2').balance, -30);
    });
...
// 6. Profiles Store
// ===========================================================================
const createProfilesStore = require('../lib/db/stores/profiles');
...
describe('ProfilesStore', () => {
  ...
  describe('profiles.getByFestival()', () => {
    it('returns profiles for a festival', async () => {
      ...
      assert.equal(result.length, 1);
      assert.equal(result[0].festivalId, 'f1');
      assert.deepEqual(result[0].picks, { s1: 1 });
    });
