...
module.exports = function createNotificationRoutes(deps) {
  ...
  const { parsePageParams } = require('../lib/pagination');
  const noopLimit = (_req, _res, next) => next();
  const rl = (max, name) => (typeof rateLimit === 'function' ? rateLimit(max, name) : noopLimit);
  ...
  router.delete('/token', userAuth, rl(10, 'notif-token-del'), validate(schemas.deleteToken), async (req, res) => {
    try {
  ...
  router.post('/read', userAuth, rl(60, 'notif-read'), validate(schemas.markRead), async (req, res) => {
    try {
  ...
  router.get('/unread', userAuth, async (req, res) => {
    try {
      ...
      const total = counts.reduce((sum, c) => sum + (c.unreadUpdates || 0), 0);
      return sendSuccess(res, {
        total,
        byFestival: counts.map((c) => ({
  ...
  router.put('/topics/:festivalId', userAuth, rl(30, 'notif-topics'), validate(schemas.topicSubscription), async (req, res) => {
    try {
      const festivalId = sanitizeIdentifier(req.params.festivalId, 100);
      if (!festivalId) return sendError(res, 400, 'Invalid festival ID', ErrorCodes.INVALID_INPUT);
      // Verify user is a member of this festival
