...
export default function mountAdminUserRoutes({ router, deps, ctx }: any): void {
    ...
    try {
      ...
      let items = users.map((user: any) => {
        ...
        return {
          ...
          totalPicks: userProfiles.reduce((count: number, profile: any) => count + Object.keys(profile.picks || {}).length, 0),
  ...
  router.post('/users/:id/roles', adminAuth, adminWriteLimit, validateParams(schemas.genericIdParams), validate(schemas.adminAddRole), async (req: any, res: any) => {
    try {
  ...
  router.delete('/users/:id/roles/:role', adminAuth, adminWriteLimit, validateParams(schemas.genericIdParams), async (req: any, res: any) => {
    try {
      const targetUserId = deps.sanitizeIdentifier(req.validatedParams.id, 100);
      if (!targetUserId) return sendError(res, 400, 'Invalid user ID', ErrorCodes.INVALID_INPUT);
  ...
  router.post('/users/:id/reset-link', adminAuth, adminWriteLimit, passwordResetRateLimit, validateParams(schemas.genericIdParams), async (req: any, res: any) => {
    try {
      const targetUserId = deps.sanitizeIdentifier(req.validatedParams.id, 100);
      if (!targetUserId) return sendError(res, 400, 'Invalid user ID', ErrorCodes.INVALID_INPUT);
  ...
  router.put('/users/:id/reset-password', adminAuth, adminWriteLimit, passwordResetRateLimit, validateParams(schemas.genericIdParams), validate(schemas.resetPassword), async (req: any, res: any) => {
    try {
      const targetUserId = deps.sanitizeIdentifier(req.validatedParams.id, 100);
      if (!targetUserId) return sendError(res, 400, 'Invalid user ID', ErrorCodes.INVALID_INPUT);
  ...
  router.delete('/users/:id', adminAuth, adminWriteLimit, validateParams(schemas.genericIdParams), async (req: any, res: any) => {
    try {
      const targetUserId = deps.sanitizeIdentifier(req.validatedParams.id, 100);
      if (!targetUserId) return sendError(res, 400, 'Invalid user ID', ErrorCodes.INVALID_INPUT);
