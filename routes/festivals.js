  ...
  router.delete('/:id', adminAuth, rateLimit(5, 'festival-delete'), async (req, res) => {
    try {
      ...
      if (!hardDelete && stores.festivals?.softDelete) {
        ...
        // Hard delete: permanently remove all data (CASCADE handles related tables)
        // chat tables dropped (migration 013) — CASCADE handles remaining related tables
        if (stores.crews?.deleteByFestival) await stores.crews.deleteByFestival(festivalId);
        // Delete the festival — CASCADE will remove stages, days, sets, profiles
