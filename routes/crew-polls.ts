...
export default function createCrewPollRoutes(deps: any) {
  ...
  // ── GET /:crewId/polls ─────────────────────────────────────────
  router.get('/:crewId/polls', userAuth, rateLimit(120, 'crew-poll-list'), validateParams(schemas.crewIdParams), async (req: any, res: any) => {
    try {
  ...
  // ── POST /:crewId/polls ────────────────────────────────────────
  router.post('/:crewId/polls', userAuth, rateLimit(10, 'crew-poll-create'), validateParams(schemas.crewIdParams), validate(schemas.pollCreate), async (req: any, res: any) => {
    try {
      const crewId = sanitizeIdentifier(req.validatedParams.crewId);
  ...
  router.post('/:crewId/polls/:pollId/vote', userAuth, rateLimit(60, 'crew-poll-vote'), validateParams(schemas.crewIdPollIdParams), validate(schemas.pollVote), async (req: any, res: any) => {
    try {
      const crewId = sanitizeIdentifier(req.validatedParams.crewId);
  ...
  router.delete('/:crewId/polls/:pollId', userAuth, rateLimit(10, 'crew-poll-delete'), validateParams(schemas.crewIdPollIdParams), async (req: any, res: any) => {
    try {
      const crewId = sanitizeIdentifier(req.validatedParams.crewId);
      const pollId = sanitizeIdentifier(req.validatedParams.pollId);
      const userId = req.user.userId;
