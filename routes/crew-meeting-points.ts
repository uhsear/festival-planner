...
export default function createCrewMeetingPointRoutes(deps: any) {
  ...
  router.put('/:crewId/home-base', userAuth, rateLimit(10, 'crew-homebase'), validateParams(schemas.crewIdParams), validate(schemas.crewHomeBase), async (req: any, res: any) => {
    try {
  ...
  router.get('/:crewId/meeting-points', userAuth, rateLimit(120, 'crew-mp-list'), validateParams(schemas.crewIdParams), async (req: any, res: any) => {
    try {
      const crewId = sanitizeIdentifier(req.validatedParams.crewId);
      const member = await stores.crews.getMember(crewId, req.user.userId);
      if (!member) return sendError(res, 403, 'Not a crew member', ErrorCodes.FORBIDDEN);
  ...
  router.post('/:crewId/meeting-points', userAuth, rateLimit(20, 'crew-mp-create'), validateParams(schemas.crewIdParams), validate(schemas.meetingPointCreate), async (req: any, res: any) => {
    try {
      const crewId = sanitizeIdentifier(req.validatedParams.crewId);
      const member = await stores.crews.getMember(crewId, req.user.userId);
      if (!member) return sendError(res, 403, 'Not a crew member', ErrorCodes.FORBIDDEN);
  ...
  router.put('/:crewId/meeting-points/:mpId', userAuth, rateLimit(20, 'crew-mp-update'), validateParams(schemas.crewIdMpIdParams), validate(schemas.meetingPointUpdate), async (req: any, res: any) => {
    try {
      const crewId = sanitizeIdentifier(req.validatedParams.crewId);
  ...
  router.delete('/:crewId/meeting-points/:mpId', userAuth, rateLimit(20, 'crew-mp-delete'), validateParams(schemas.crewIdMpIdParams), async (req: any, res: any) => {
    try {
      const crewId = sanitizeIdentifier(req.validatedParams.crewId);
      const mpId = sanitizeIdentifier(req.validatedParams.mpId);
