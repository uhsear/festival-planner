 ...
 */
async function collectGdprData(userId: any, currentUser: any, deps: any) {
  ...
  if (stores?.crews?.listForUser) {
    try {
      ...
      exportData.crews = (userCrews || []).map((c: any) => ({
        id: c.id,
        name: c.name,
  ...
  if (stores?.sessions?.listUserSessions) {
    try {
      ...
      exportData.sessions = sessions.map((s: any) => ({
...
export default function createAccountRoutes(deps: any) {
  ...
  router.post('/avatar', userAuth, rateLimit(10, 'avatar-upload'), handleAvatarUpload, async (req: any, res: any) => {
    try {
  ...
  router.delete('/avatar', userAuth, rateLimit(60, 'avatar-delete'), async (req: any, res: any) => {
    try {
      const updatedUser = await runUserTask(req.user.userId, async () => {
        const currentUser = await getUserById(req.user.userId);
        if (!currentUser) return null;
