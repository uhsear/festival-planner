...
export default function FestivalModeSkeleton() {
  return (
    <div
      ...
      aria-busy="true"
      aria-label="Loading festival mode"
    >
      ...
      {(['NOW', 'UP NEXT'] as const).map((label) => (
        <section key={label} className="mb-5" aria-label={`Loading ${label}`}>
          <h2
            className="text-xs font-bold uppercase tracking-[0.08em] text-text-secondary mb-2 leading-[1.15] font-display"
            aria-hidden="true"
          >
          ...
          {[0, 1].slice(0, label === 'NOW' ? 1 : 2).map((i) => (
            <div
              key={i}
              className="fm-card-enter block w-full text-left py-3.5 px-4 bg-glass rounded-xl border border-[var(--color-aqua-a08)] mb-2 flex flex-col gap-2"
            >
