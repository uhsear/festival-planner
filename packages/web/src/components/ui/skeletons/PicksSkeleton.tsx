...
export default function PicksSkeleton() {
  return (
    <div className="pb-5" aria-busy="true" aria-label="Loading picks" role="region">
      {[
        { label: 'Must See', count: 3, dot: 'var(--color-priority-must)' },
        { label: 'Want to See', count: 2, dot: 'var(--color-priority-want)' },
        { label: 'Maybe', count: 2, dot: 'var(--color-priority-maybe)' },
      ].map((section) => (
        <div key={section.label} className="mb-4 md:grid md:grid-cols-2 md:gap-x-4">
          ...
          {Array.from({ length: section.count }).map((_, i) => (
            <div
              key={i}
              className="pick-item grid grid-cols-[auto_1fr_auto_auto] items-center gap-x-3 gap-y-2 px-4 py-3 bg-bg-card backdrop-blur-[8px] border border-border rounded-sm mb-1.5 flex flex-col gap-1.5 pointer-events-none"
            >
