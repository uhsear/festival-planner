...
function createRateLimiters({
   ...
   *
   * @param {object}   opts
   * @param {object}   opts.redisClient  - Redis client (scoped path) or null
   * @param {string}   opts.redisKey     - Key used for the Redis rate check
   * @param {number}   opts.max          - Max requests in window
   * @param {number}   opts.windowMs     - Window duration in ms
   * @param {Map}      opts.fallbackMap  - In-memory map for fallback counters
   * @param {string}   opts.fallbackKey  - Key used in the in-memory fallback map
   * @param {string}   opts.tier         - 'scoped' | 'global' (controls Redis path)
   */
  async function tryRedisRateCheck({ redisClient, redisKey, max, windowMs, fallbackMap, fallbackKey, tier }) {
...
function createPasswordResetRateLimit(config, { log, sendError, ErrorCodes, redis } = {}) {
   ...
   */
  async function tryRedis(emailKey) {
    if (!redis) return null;
    try {
      const { redisRateCheck } = require('./redis');
      const redisKey = `${REDIS_PREFIX}rl:pw-reset:${emailKey}`;
      return await redisRateCheck(redis, redisKey, MAX_ATTEMPTS, WINDOW_MS);
    } catch {
  ...
  return async function passwordResetRateLimit(req, res, next) {
    ...
    const emailHash = require('crypto').createHash('sha256').update(email).digest('hex').slice(0, 32);
