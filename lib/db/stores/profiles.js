    ...
    async replaceAll(nextProfiles) {
      return withTransaction(pool, async (client) => {
        ...
        for (const profile of nextProfiles) {
          ...
          // #24: Sync normalized picks table (batch)
          await client.query('DELETE FROM festival_profile_picks WHERE profile_id = $1', [profile.id]);
          const picks = profile.picks || {};
          const pickRows = Object.entries(picks).map(([setId, priority]) => [profile.id, setId, priority]);
          await batchInsert(client, 'festival_profile_picks', ['profile_id', 'set_id', 'priority'], pickRows);

          // #24: Sync normalized notes table (batch)
          await client.query('DELETE FROM festival_profile_notes WHERE profile_id = $1', [profile.id]);
          const notes = profile.notes || {};
          const noteRows = Object.entries(notes).map(([setId, text]) => [profile.id, setId, text]);
      ...
      if (rows.length > 0) {
        ...
        await withTransaction(pool, async (client) => {
          ...
          for (const pid of profileIds) {
            await client.query('DELETE FROM festival_profile_picks WHERE profile_id = $1', [pid]);
            await client.query('DELETE FROM festival_profile_notes WHERE profile_id = $1', [pid]);
          }
        });
      }
