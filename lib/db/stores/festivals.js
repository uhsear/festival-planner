 ...
 * @param {Array<Array>} rows - Array of row arrays, each with the same column count
 * @param {number} [startIdx=1] - Starting $N index (1-based)
 * @returns {{ clause: string, params: Array }} e.g. { clause: '($1,$2),($3,$4)', params: [...] }
 */
function buildMultiInsert(rows, startIdx = 1) {
  const params = [];
  let idx = startIdx;
 ...
 */
function collectFestivalChildren(festivalId, festival) {
  const stageRows = (festival.stages || []).map((stage, sortOrder) =>
...
async function insertStagesBatch(client, rows) {
...
async function insertDaysBatch(client, rows) {
  if (rows.length === 0) return;
  const { clause, params } = buildMultiInsert(rows);
  await client.query(
...
async function insertSetsBatch(client, rows) {
  if (rows.length === 0) return;
  const { clause, params } = buildMultiInsert(rows);
  await client.query(
...
async function restorePicksBatch(client, pickRows, newSetIds) {
  const surviving = pickRows.filter((p) => newSetIds.has(p.set_id));
  if (surviving.length === 0) return;
  const rows = surviving.map((p) => [p.profile_id, p.set_id, p.priority]);
  const { clause, params } = buildMultiInsert(rows);
  await client.query(
...
async function restoreRatingsBatch(client, ratingRows, setIdentityMap) {
  const mapped = [];
 ...
 */
async function replaceChildRows(client, festivalId, children) {
  ...
  const festivals = {
    ...
    async hardDelete(festivalId) {
      return withTransaction(pool, async (client) => {
        ...
        await client.query('DELETE FROM festival_profile_reminders WHERE set_id IN (SELECT id FROM festival_sets WHERE festival_id = $1)', [festivalId]);
    ...
    async insertSets(festivalId, sets) {
      if (!sets || sets.length === 0) return;
      const rows = sets.map((set, sortOrder) => [
        set.id, festivalId, set.dayIndex, set.artist, set.stageId,
        set.startTime, set.endTime, set.sortOrder ?? sortOrder,
        set.linkUrl || null, JSON.stringify(set.artists || []),
      ]);
      await insertSetsBatch({ query: (sql, params) => pool.query(sql, params) }, rows);
    },
    ...
    async update(festivalId, fields) {
      ...
      return withTransaction(pool, async (client) => {
        ...
        if (fields.stages !== undefined) {
          await client.query('DELETE FROM festival_stages WHERE festival_id = $1', [festivalId]);
          const stageRows = (fields.stages || []).map((stage, sortOrder) =>
            [festivalId, stage.id, stage.name, stage.color, sortOrder],
          );
          await insertStagesBatch(client, stageRows);
        }
