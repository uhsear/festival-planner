...
import { withTransaction, serializeJson } from '../connection';

// Batch INSERT helper — builds multi-row VALUES clause
function batchInsert(client: any, table: string, columns: string[], rows: any[][]) {
...
export default function createProfilesStore(pool: Pool, utils: any) {
  ...
  const profiles = {
    ...
    async userIdsByFestival(festivalId: string) {
      const result = await pool.query(
    ...
    async readByUserAndFestival(userId: string, festivalId: string) {
      const result = await pool.query(
    ...
    async replaceAll(nextProfiles: any[]) {
      return withTransaction(pool, async (client) => {
        ...
        const profileIds = nextProfiles.map((profile: any) => profile.id);
        await client.query(
          `UPDATE festival_profiles SET deleted_at = NOW(), updated_at = NOW() WHERE deleted_at IS NULL AND id NOT IN (${profileIds.map((_: any, i: number) => `$${i + 1}`).join(',')})`,
          profileIds,
        );

        // Batch UPSERT all profiles in a single multi-row query
        const upsertValues: any[] = [];
        const upsertPlaceholders = nextProfiles.map((profile: any, ri: number) => {
        ...
        const allPickRows: any[][] = [];
        const allNoteRows: any[][] = [];
    ...
    async create({ id, festivalId, userId, name, picks, notes, reminders, createdAt }: any) {
...
`,
    ...
    async update(profileId: string, fields: any) {
    ...
    async delete(profileId: string, { deletedBy, reason }: { deletedBy?: string; reason?: string } = {}) {
