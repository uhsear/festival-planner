...
function createNotificationsStore(pool, _utils) {
  const deviceTokens = {
    async register({ id, userId, token, platform, deviceName }) {
  ...
  const notificationPrefs = {
    ...
    async upsert({ userId, crewUpdates, setReminders, scheduleChanges, dndStart, dndEnd }) {
      await pool.query(`
  ...
  const notificationCounts = {
    ...
    async increment(userId, festivalId, field) {
