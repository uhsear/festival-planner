...
function createExpensesStore(pool) {
  return {
    ...
    async getByCrew(crewId) {
      ...
      return rows.map(r => ({
        ...r,
        split_with: typeof r.split_with === 'string' ? JSON.parse(r.split_with) : (r.split_with || []),
      }));
    },

    async getById(expenseId) {
      const { rows } = await pool.query('SELECT * FROM crew_expenses WHERE id = $1', [expenseId]);
      return rows[0] || null;
    },
    ...
    async getBalances(crewId) {
      ...
      const { rows: expenses } = await pool.query(
        'SELECT * FROM crew_expenses WHERE crew_id = $1', [crewId]
      );
      ...
      for (const exp of expenses) {
        const splitWith = typeof exp.split_with === 'string' ? JSON.parse(exp.split_with) : (exp.split_with || []);
