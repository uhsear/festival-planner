...
function createUsersStore(pool, utils) {
  ...
  const users = {
    ...
    async getByUsername(username) {
      const result = await pool.query(`SELECT ${USER_COLUMNS} FROM users WHERE LOWER(username) = LOWER($1) AND deleted_at IS NULL LIMIT 1`, [username]);
      return normalizeRow(result.rows[0]);
    },
    ...
    async hardDelete(userId) {
      ...
      // so we must delete child rows explicitly.
      await pool.query('DELETE FROM set_ratings WHERE user_id = $1', [userId]);
      await pool.query('DELETE FROM notification_topic_subs WHERE user_id = $1', [userId]);
      ...
      await pool.query('DELETE FROM notification_counts WHERE user_id = $1', [userId]);
      ...
      await pool.query('DELETE FROM festival_profiles WHERE user_id = $1', [userId]);
      await pool.query('DELETE FROM crew_members WHERE user_id = $1', [userId]);
      await pool.query('DELETE FROM crew_expenses WHERE paid_by = $1', [userId]);
      await pool.query('DELETE FROM crew_activity WHERE user_id = $1', [userId]);
