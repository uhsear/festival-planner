...
function createRatingsStore(pool) {
  return {
    async upsert(userId, setId, rating, note = '') {
    ...
    async getByUser(userId, festivalId) {
      const result = await pool.query(`
    ...
    async getByFestival(festivalId, { cursor, limit = 50 } = {}) {
      const params = [festivalId, limit + 1];
      let having = '';
      if (cursor) {
    ...
    async getCrewRatings(crewId, festivalId, { cursor, limit = 50 } = {}) {
      const params = [crewId, festivalId, limit + 1];
      let cursorClause = '';
      if (cursor) {
...
module.exports = { createRatingsStore };
