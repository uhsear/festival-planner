...
function createCrewsStore(pool, _utils) {
  ...
  const crews = {
    ...
    async getById(crewId) {
      const result = await pool.query(`
        SELECT id, festival_id AS "festivalId", name, created_by AS "createdBy", invite_code AS "inviteCode", max_members AS "maxMembers", home_base_location AS "homeBaseLocation", home_base_time AS "homeBaseTime", home_base_updated_at AS "homeBaseUpdatedAt", created_at AS "createdAt", updated_at AS "updatedAt"
    ...
    async getByInviteCode(code) {
      const result = await pool.query(`
        SELECT id, festival_id AS "festivalId", name, created_by AS "createdBy", invite_code AS "inviteCode", max_members AS "maxMembers", home_base_location AS "homeBaseLocation", home_base_time AS "homeBaseTime", home_base_updated_at AS "homeBaseUpdatedAt", created_at AS "createdAt", updated_at AS "updatedAt"
        FROM crews WHERE invite_code = $1 AND (invite_expires_at IS NULL OR invite_expires_at > NOW())
      `, [code]);
      return result.rows[0] || null;
    },
    ...
    async listByFestival(festivalId) {
      const result = await pool.query(`
        SELECT id, festival_id AS "festivalId", name, created_by AS "createdBy", invite_code AS "inviteCode", max_members AS "maxMembers", home_base_location AS "homeBaseLocation", home_base_time AS "homeBaseTime", home_base_updated_at AS "homeBaseUpdatedAt", created_at AS "createdAt", updated_at AS "updatedAt"
        FROM crews WHERE festival_id = $1
