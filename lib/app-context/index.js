...
const { createRedisClient, createRedisRateLimiter, createRedisPresenceStore, createRedisCircuitBreaker, redisRateCheck } = require('../redis');
...
const { createCacheHelpers } = require('./cache');
const { createSessionHelpers } = require('./session');
...
function createAppContext(overrides = {}) {
  ...
  // ── Database ──────────────────────────────────────────────────────────
  const { pool } = openPlannerDatabase({ databaseUrl: config.DATABASE_URL, log, poolSize: config.DB_POOL_SIZE, poolMin: config.PG_POOL_MIN });
  ...
  const emitProfileIdentityWrapped = (user, io) => emitProfileIdentity(user, io, getProfiles);
