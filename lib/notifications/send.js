...
const { loadConfig: _loadSendConfig } = require('../config');
 ...
 */
function isValidDeviceToken(device) {
 ...
 */
function enforcePayloadSize(msg, log, context = '') {
 ...
 */
async function resolveFestivalTargets(stores, festivalId, excludeUserIds) {
  ...
  return festivalUserIds.filter((uid) => !excludeSet.has(uid));
}
...
function createSendService({ stores, config, log, messaging, retryQueue }) {
   ...
   */
  async function sendToDevice({ device, fcmMessage, userId, type, safeTitle, safeBody, safeData, retryQueue: deviceRetryQueue }) {
  ...
  async function sendToOfflineUsers({ festivalId, type, title, body, data = {}, excludeUserIds = [], topic = null, threadId = null }) {
    ...
    const filteredIds = targetUserIds.filter((uid) => !topicUnsubscribed.has(uid));
