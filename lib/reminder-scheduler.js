...
const FIRE_WINDOW_MS = 65_000; // slight overlap to prevent gaps

function createReminderScheduler({ pool, stores, notificationService, log, config: _config }) {
  ...
  const _firedCache = new Map(); // `${userId}:${setId}`  timestamp (in-memory dedup)
  const DEDUP_TTL_MS = 120 * 60_000; // 2 hours
   ...
   */
  function computeSetStartMs(dayDate, startTime) {
  ...
  async function tick() {
    try {
      ...
      for (const festival of festivals) {
        ...
        const dayMap = new Map(daysResult.rows.map(d => [d.day_index, d]));
        const stageMap = new Map(stagesResult.rows.map(s => [s.id, s]));
        ...
        for (const set of setsResult.rows) {
          ...
          if (set.artists && Array.isArray(set.artists) && set.artists.length > 0) {
            artistName = set.artists.map(a => a.name || a).join(' b2b ');
          } else {
        ...
        for (const profile of profiles) {
          ...
          try {
            ...
            log.warn('reminder json parse failed, skipping profile', { userId: profile.userId, error: err.message });
            continue;
          }
          ...
          for (const [setId, minutesBefore] of Object.entries(reminders)) {
            ...
            const fireAt = setInfo.startMs - (minutesBefore * 60_000);
            if (fireAt < now || fireAt > now + FIRE_WINDOW_MS) continue;
            ...
            try {
              ...
              // DND check
              const { isInDndWindow } = require('./notifications');
            ...
            try {
              ...
              log.debug('reminder fired', { userId: profile.userId, setId, minutesBefore });
            } catch (err) {
              log.warn('reminder send failed', { userId: profile.userId, setId, error: err.message });
            }
          }
        }
      }
