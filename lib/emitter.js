...
function createSocketEmitter({ io, log, notificationService, buildAvatarUrl, _getUserById }) {
  ...
  function _flushProfileBatch(key) {
  ...
  function profileCreated({ festivalId, profile, user }) {
  ...
  function profileUpdated({ profile, user, changedFields = {} }) {
    ...
    if (notificationService?.isConfigured && typeof notificationService.sendToOfflineUsers === 'function' && changedFields.picks) {
      ...
      }).catch(err => log.warn('crew push failed', { error: err.message }));
      ...
      notificationService.sendSilentSync?.({
        ...
        syncType: 'profiles',
        excludeUserIds: [profile.userId],
      }).catch(err => log.warn('silent sync push failed', { error: err.message }));
    }
  }

  function profileDeleted({ festivalId, profileId }) {
  ...
  function festivalCreated({ id, name }) {
    io.emit('festival:created', { _v: EVENT_VERSION, id, name });
  }

  function festivalUpdated({ festival }) {
    ...
    if (notificationService?.isConfigured && typeof notificationService.sendToOfflineUsers === 'function') {
      ...
      }).catch(err => log.warn('schedule push failed', { error: err.message }));
  ...
  function festivalDeleted({ id }) {
    io.emit('festival:deleted', { _v: EVENT_VERSION, id });
  }



  function presenceUpdate({ festivalId, online }) {
