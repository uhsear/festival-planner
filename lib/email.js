...
function _idempotencyKey(to, subject) {
  return `${to}:${subject}`;
}

function _checkIdempotency(to, subject, log) {
...
function _recordSend(to, subject) {
  const key = _idempotencyKey(to, subject);
 ...
 */
async function sendEmail({ to, subject, html, text, config, log }) {
...
function _wrapTemplate(title, bodyHtml) {
  return `<!DOCTYPE html>
<html lang="en">
...
function _buttonHtml(url, label) {
 ...
 */
async function sendPasswordResetEmail({ to, username, resetUrl, config, log }) {
  const bodyHtml = `
  ...
  return sendEmail({ to, subject: 'Reset your Festie password', html, text, config, log });
}

/**
 * Send email verification email.
 */
async function sendVerificationEmail({ to, username, verifyUrl, config, log }) {
  const bodyHtml = `
    <p style="color:#d4d4d8;font-size:15px;line-height:1.6;margin:0 0 16px">Hey <strong style="color:#e4e4e7">${escapeHtml(username)}</strong>,</p>
  ...
  return sendEmail({ to, subject: 'Verify your Festie email', html, text, config, log });
}
