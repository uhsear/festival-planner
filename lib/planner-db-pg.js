...
const {
  ...
  openPlannerDatabase: openPlannerDatabaseRaw,
  parseJsonObject,
  serializeJson,
  withTransaction,
} = dbIndex;
 ...
 *
 * @type {Map<string, Promise<{applied: number[], current: number}>>}
 */
const MIGRATED_URLS = new Map();

/**
 * Discover migration files on disk, sorted by (version, suffix).
 *
 * @returns {Array<{version: number, suffix: string, sortKey: string, file: string, fullPath: string}>}
 */
 ...
 *
 * @param {import('pg').PoolClient} client
 * @param {{version: number, suffix: string, file: string, fullPath: string}} migration
 * @param {(level: string, msg: string, meta?: object) => void} log
 */
async function applyOneMigration(client, migration, log) {
 ...
 * @returns {Promise<{applied: number[], current: number, gap: number[]}>}
 */
async function runPostgresMigrations(pool, opts = {}) {
  const log = opts.log || (() => {});
  ...
  try {
    ...
    return { applied: appliedVersions, current, gap };
  } finally {
    client.release();
  }
}
 ...
 * extension.
 *
 * @param {object} opts - Forwarded to the underlying openPlannerDatabase
 * @returns {{pool: import('pg').Pool, migrationsReady: Promise<object>}}
 */
function openPlannerDatabase(opts = {}) {
  ...
  if (!migrationsReady) {
    migrationsReady = runPostgresMigrations(pool, { log }).catch((err) => {
 ...
 */
const __resetMigrationCacheForTests = () => { MIGRATED_URLS.clear(); };
