...
const { sanitizeString, sanitizeIdentifier } = require('./sanitize');
 ...
 * @param {Object} config - Config with MAX_STAGES, MAX_DAYS, MAX_SETS_PER_DAY
 * @param {Object} body - Festival payload to validate
 * @returns {Array} - Array of error strings (empty if valid)
 */
function validateFestival(config, body) {
  const errors = [];
  ...
  for (const day of body.days || []) {
    ...
    for (const set of day.sets || []) {
      ...
      const artist = set?.artists?.length > 0
        ? set.artists.map((a) => sanitizeString(a.name || '', 300)).filter(Boolean).join(' b2b ')
    ...
    for (const stageEntries of stageSchedules.values()) {
      const sorted = stageEntries
        .map((entry) => {
          const start = entry.start.split(':').map((part) => Number.parseInt(part, 10));
          const end = entry.end.split(':').map((part) => Number.parseInt(part, 10));
