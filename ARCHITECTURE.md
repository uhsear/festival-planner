...
- **PM2 cluster mode** (`ecosystem.config.cjs`) with 4 workers
...
`PUBLIC_ORIGIN`, `DATABASE_URL`, `SESSION_SECRET`, `FIREBASE_CREDENTIALS_PATH`, `RESEND_API_KEY`; `REDIS_URL` for cluster mode. All read through `lib/config.ts`.
