...
BACKUP_FILE="${BACKUP_DIR}/fp_${TIMESTAMP}.sql.gz"
...
# Dump and compress
if PGPASSWORD="$DB_PASS" pg_dump -h "$DB_HOST" -U "$DB_USER" "$DB_NAME" | gzip > "$BACKUP_FILE"; then
...
find "$BACKUP_DIR" -name "fp_*.sql.gz" -mtime +${DAILY_KEEP} -not -name "fp_*_weekly.sql.gz" -delete 2>/dev/null

# Tag Sunday backups as weekly
DOW=$(date +%u)
if [ "$DOW" = "7" ]; then
  WEEKLY_FILE="${BACKUP_DIR}/fp_${TIMESTAMP}_weekly.sql.gz"
  cp "$BACKUP_FILE" "$WEEKLY_FILE"
  echo "[$(date -Iseconds)] Weekly backup: $WEEKLY_FILE" >> "$LOG_FILE"
fi

# Prune old weekly backups (keep last 4)
ls -t "$BACKUP_DIR"/fp_*_weekly.sql.gz 2>/dev/null | tail -n +$((WEEKLY_KEEP + 1)) | xargs rm -f 2>/dev/null
